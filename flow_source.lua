...
function flow:DeepMerge(target, source)
...
function flow:UpdateTheme(Config)
...
function flow:BindFrame(frame, properties)
	if binds[frame] then
...
function flow:Modify(frame, properties)
	local parts = flow:GetBoundParts(frame)
	if parts then
		for propName, propValue in pairs(properties) do
			for _, pt in pairs(parts) do
				pt[propName] = propValue
...
function flow:UnbindFrame(frame)
...
function flow:HasBinding(frame)
	return binds[frame] ~= nil
end

-- Returns an array using this.
function flow:GetBoundParts(frame)
	return binds[frame] and binds[frame].parts
end



function flow:GetDark(Color, val, mode)
...
function flow:GetLighter(color, strength)
...
function flow:ColorUnpack(color)
...
function flow:HidePH(instance, placeholder, recursive)
...
function flow:AddConnection(Type, Callback)
	...
	flow.Connections = flow.Connections or {}
	table.insert(flow.Connections, ConnectionData)
	...
	local function Disconnect()
		...
		for i = #flow.Connections, 1, -1 do
			if flow.Connections[i] == ConnectionData then
				table.remove(flow.Connections, i)
				break
			end
		end
	end
	...
	task.spawn(function()
		task.wait(10)
		for i = #flow.Connections, 1, -1 do
			if not flow.Connections[i].Connection.Connected then
				table.remove(flow.Connections, i)
			end
		end
	end)
...
function flow:MakeResizable(Dragger, Object, MinSize, Callback, LockAspectRatio)
	...
	flow:AddConnection(Dragger.InputBegan, onInputBegan)
	flow:AddConnection(userInput.InputChanged, onInputChanged)
	flow:AddConnection(Dragger.InputEnded, onInputEnded)
end


local loadTweens = {}

function flow:registerLoadTween(object, properties, initialState, tweenInfo)
...
function flow:resetToInitialState(animated, resetTweenInfo)
...
function flow:replayLoadTweens(targetObject)
	flow:resetToInitialState(false)
...
function flow:removeLoadTween(object)
...
function flow:WiggleText(label)
...
function flow:StopWiggle(label)
...
function flow:updateLayout(container, spacing)
...
function flow:AddDrag(Object, Main, ConstrainToParent)
	...
	flow:AddConnection(Object.InputBegan, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging = true
	...
	flow:AddConnection(userInput.InputChanged, function(input)
	...
	flow:AddConnection(Object.InputEnded, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging = false
		end
	end)
...
function flow:HidePlaceHolder(instance, placeholder, recursive)
	if typeof(instance) ~= "Instance" or type(placeholder) ~= "string" then
		warn("[removeplaceholder] Invalid input: Expected (Instance, string)")
		return
	end
...
do
	...
	function flow:Load(Config)
		task.wait(1)
		print('[FLOW] Loader Loaded')
		local LOADER = Loader
		LOADER.Enabled = true
		LOADER.Parent = coregui

		-- PreLoad
		Config.Name = Config.Name or 'Flow™'
		Config.Logo = Config.Logo or 'rbxassetid://14554547135'
		Config.ConfigFolder = Config.ConfigFolder or 'Flow'
		Config.Status = Config.Status or false
		Config.Accent = Config.Accent or flow.theme.Accent
		Config.HitBox = Config.HitBox or flow.theme.HitBox
		...
		local LoaderConfig = {
			...
			Accent = Config.Accent or flow.theme.Accent;
			Hitbox = Config.HitBox or flow.theme.HitBox;
			Socials = {}
		}
		...
		--	LOADER.load.info.build.Text = flow.Build
		...
		local function load()
			...
			if Config.ConfigurationSaving and Config.ConfigurationSaving.Enabled then
				local folderName = Config.ConfigurationSaving.FolderName or "FlowSec"
				local fileName = Config.ConfigurationSaving.FileName or "default_config"
				local fullPath = folderName .. "/" .. fileName .. ".lua"

				flow.ConfigEnabled = true
				flow.ConfigFolder = folderName  
				flow.ConfigFile = fileName     
				...
				if isfolder and not isfolder(folderName) then
					...
					if not success then
						warn("[FLOW] Failed to create folder:", err)
					end
				end
				...
				if not success then
					warn("[FLOW] File write/read blocked or failed:", err)
				end
			end
			...
			local UI_TAG = "flowUILoader"
			local MARKER_NAME = "FLOWUIDetector"
			local INTERNAL_UUID = ("FLOW-" .. tostring(game.JobId):gsub("-", "") .. tostring(tick())):gsub("%.", "")
			...
			if not successLibrary or not Library then
				warn("[FLOW] Failed to load Library.")
				return
			end
			...
			task.spawn(function()
				while Library and Library.Parent do
					...
					if Library.Parent ~= coregui then
						warn("Flow 〡 UI moved. Restoring...")
						pcall(function()
							Library.Parent = coregui
						end)
					end
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				local rootFolder = Config.ConfigurationSaving and Config.ConfigurationSaving.FolderName or "FlowSec"
				...
				warn("[FLOW] Discord AutoJoin enabled. Preparing to send invite:", inviteCode)
				...
				if isfolder and not isfolder(discordFolder) then
					...
					if folderSuccess then
						print("[FLOW] Created DiscordInvites folder at:", discordFolder)
					else
						warn("[FLOW] Failed to create DiscordInvites folder:", folderErr)
					end
				end

				local shouldPrompt = true
				if isfile and discordConfig.RememberJoins and isfile(inviteFilePath) then
					print("[FLOW] Discord invite '" .. inviteCode .. "' already joined. Skipping prompt.")
					shouldPrompt = false
				end
				...
				if shouldPrompt then
					if request then
						...
						if reqSuccess then
							print("[FLOW] Discord invite sent successfully to:", inviteCode)
						else
							warn("[FLOW] Failed to send Discord invite:", reqErr)
						end
					else
						warn("[FLOW] Request function not available — cannot send Discord invite.")
					end
					...
					if discordConfig.RememberJoins then
						...
						if writeSuccess then
							print("[FLOW] Logged Discord invite join for:", inviteCode)
						else
							warn("[FLOW] Failed to write join log for invite:", writeErr)
						end
					end
				end
			end
		...
		flow.theme.Accent = Config.Accent;
		flow.theme.HitBox = Config.HitBox;
		LOADER:Destroy()
...
local function ApplyFlag(Flag, Value)
	...
	if Flag.Type == "ColorPicker" then
		local unpacked = flow:ColorUnpack(Value)
	...
	warn("Flow 〡 Unsupported flag type for:", Flag)
	return false
end

function LoadConfig(Configuration)
	if type(Configuration) ~= "string" then
		warn("Flow 〡 Invalid config format.")
		return false
	end

	local Decoded = SafeDecode(Configuration)
	if not Decoded then
		warn("Flow 〡 Failed to decode config.")
		return false
	end

	-- Version check (future ready)
	if Decoded._version and Decoded._version ~= CONFIG_VERSION then
		warn("Flow 〡 Config version mismatch.")
	end

	local changed = false

	for FlagName, Flag in pairs(flow.Flags) do
...
local function PerformSave()
	if not flow.ConfigEnabled then return end
	...
	for FlagName, Flag in pairs(flow.Flags) do
		if Flag.Type == "ColorPicker" then
			Data[FlagName] = flow:ColorPack(Flag.Color)
		else
			Data[FlagName] =
	...
	-- Ensure folder exists
	if makefolder and not isfolder(flow.ConfigFolder) then
		makefolder(flow.ConfigFolder)
	end

	local path = string.format("%s/%s.json", flow.ConfigFolder, flow.ConfigFile)
	...
	if not success then
		warn("Flow 〡 Failed to save config:", err)
	end
end
...
function flow:Notify(Notification)
	task.spawn(function()
		...
		if NotifData.Animation == 'Wiggle' then
			flow:WiggleText(Notification.Title)
		end
...
flow:HidePH(tabs, 'btn')
flow:HidePH(pages, 'page')

--@@Initialize
function flow:Init(library)
	...
	--dragging
	flow:AddDrag(top, window, true)
	flow:MakeResizable(window.resize, window, Vector2.new(454, 228))
	...
	flow:AddConnection(flow.Comms.Event, function(p, value)
		if p == "Accent" then
			for i, glow in pairs(window.clipframe:GetChildren()) do
				if glow:IsA("ImageLabel") then
	...
	--[[	local executor = "Undetected"
		...
		local FILE = flow.ConfigFolder .. "/last_game.json"
		...
		local function LoadLastGame()
			if not flow.ConfigEnabled then return nil end
		...
		task.delay(120, function()
			if flow and flow.ConfigEnabled then
	...
	do
		...
		function settings:inittab(tab)
			...
			function telement:Toggle(Toggle)
				...
				local function UpdateToggleUI(state)
					local targetColor = state and flow.theme.HitBox or Color3.fromRGB(28, 28, 28)
				...
				flow:AddConnection(flow.Comms.Event, function(p, color)
					if p == 'HitBox' then
						if data.V then
							task.wait(0.5)
				...
				if data.SFlag then
					flow.SettingsFlags[data.SFlag] = data
				end
			end
			...
			function telement:Keybind(Keybind)
				...
				-- Main input handler
				flow:AddConnection(userinput.InputBegan, function(input)
			...
			function telement:ColorPicker(ColorPicker)
				...
				flow:AddConnection(SVPicker.InputBegan, function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				flow:AddConnection(SVPicker.InputEnded, function(i)
				...
				flow:AddConnection(HUESlider.InputBegan, function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				flow:AddConnection(HUESlider.InputEnded, function(i)
				...
				if flow.ConfigEnabled and data.Flag and flow.Flags[data.Flag] then
					local existing = flow.Flags[data.Flag]
			...
			function telement:Slider(Slider)
				...
				for _, Options in ipairs(data.Sliders) do
					...
					flow:replayLoadTweens(Slider.slide.slideframe)
					...
					flow:AddConnection(userinput.InputEnded, function(input, processed)
						if input.UserInputType == Enum.UserInputType.MouseButton1  or input.UserInputType == Enum.UserInputType.Touch then
							dragging = false
					...
					Slider.slide.slideframe.BackgroundColor3 = flow.theme.HitBox
					Slider.slide.slideframe.shadowHolder.ambientShadow.ImageColor3 = flow.theme.HitBox
					Slider.slide.slideframe.shadowHolder.penumbraShadow.ImageColor3 = flow.theme.HitBox
					Slider.slide.slideframe.shadowHolder.umbraShadow.ImageColor3 = flow.theme.HitBox
					...
					if Options.SFlag then
						if Options.SFlag then
							flow.SettingsFlags[Options.SFlag] = Options
						end
					end
		...
		local seq = flow.theme.DropShadow
	...
	--[[	a:ColorPicker({
		...
		function flow:SaveSettingsConfig()
			local Data = {}


			for flag, v in pairs(flow.SettingsFlags or {}) do

				if v.Type == "ColorPicker" and v.Color then
					Data[flag] = flow:ColorPack(v.Color)
				elseif v.V ~= nil then
					Data[flag] = v.V
			...
			local path = string.format("%s/SettingsConfig.lua", flow.ConfigFolder)
		...
		function flow:LoadSettingsConfig()
			local path = string.format("%s/SettingsConfig.lua", flow.ConfigFolder)
			...
			for flag, val in pairs(data) do
				local setting = flow.SettingsFlags and flow.SettingsFlags[flag]
				if setting then
					if setting.Set then
						setting:Set(val)
					elseif setting.Type == "ColorPicker" and setting.Color then
						setting.Color = flow:ColorUnpack(val)
		...
		ss = flow:AddConnection(runservice.Heartbeat, function()
	...
	function tbdata:InitTab(tab)
		...
		if tbdata.first then
			...
			tweenservice:Create(Tab.indicator, TweenInfo.new(2, Enum.EasingStyle.Exponential), { BackgroundColor3 = flow.theme.Accent }):Play()
			tweenservice:Create(Tab.indicator.glow, TweenInfo.new(2, Enum.EasingStyle.Exponential), { ImageColor3 = flow.theme.Accent }):Play()
		...
		local function ApplyTabStyle(tabButton, isSelected)
			...
			local targetColor = isSelected and flow.theme.Accent or Color3.fromRGB(29, 29, 29)
		...
		local function SwitchToTab(tabName)
			...
			if not (selectedTab and targetPage) then
				warn("[Flow] SwitchToTab failed:", tabName)
				return
			end
		...
		local function createResult(page, func)
			...
			result.interact.MouseButton1Click:Connect(function()
				...
				tweenservice:Create(func, TweenInfo.new(0.2), {
					BackgroundColor3 = flow:GetLighter(original, 0.03)
				}):Play()
	...
	--[[	if Data.Home.Enabled then
		...
		function initelement:Toggle(Toggle)
			...
			if flow.ConfigEnabled and data.Flag and flow.Flags[data.Flag] and flow.Flags[data.Flag].V ~= nil then
				data:Set(flow.Flags[data.Flag].V, true)
			end

			if flow.ConfigEnabled then
				if data.Flag then
					flow.Flags[data.Flag] = data
				end
			end
			...
			for _, Options in ipairs(data.Sliders) do
				...
				if flow.ConfigEnabled then
					if Options.Flag then
						flow.Flags[Options.Flag] = Options
					end
				end
		...
		function flow:LoadSaveConfig()
			...
			if flow.ConfigEnabled then
				...
				task.wait(1)
				local success, result = pcall(function()
					local filePath = string.format("%s/%s.json", flow.ConfigFolder, flow.ConfigFile)
				...
				elseif not success then
					warn("[FLOW] Configurations Error " .. tostring(result))
				end
			end
		end
